...
void SlotMachine::Update()
{
    ...
    // i assume player state static in v4 also disables collisions, but here it doesnt, so i have to disable the hitbox box for the slot entirely when
    // the player is supposed to be in it
...
void SlotMachine::State_Reward()
{
    ...
    if (this->rewardRingCount == 0) {
        ...
        int32 prizePosX = 0;
        int32 prizePosY = 0;
        prizePosX       = GET_BIT(Zone::sVars->timer, 0);
        if (prizePosX == false) {
