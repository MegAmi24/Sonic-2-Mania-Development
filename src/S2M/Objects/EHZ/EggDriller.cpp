...
void EggDriller::Update()
{
    ...
    if (this->type == Drill) {
        if (car->state.Matches(&EggDriller::Car_Driving) || car->state.Matches(&EggDriller::Car_Explode)
            || car->state.Matches(&EggDriller::Car_Destroyed)) { // this makes the drill hitbox only active after the boss sets up
...
void EggDriller::Car_SelfDriving()
{
    ...
    if ((Zone::sVars->timer &= 15) == 0) {
        ...
        exhaustPos.y     = this->position.y - TO_FIXED(14);
...
void EggDriller::Car_Driving()
{
    ...
    if (this->position.x <= this->boundsL) {
        ...
        this->direction ^=
            FLIP_X; // whats the difference between ^= and =??? doing this->direction ^= FLIP_X works but not when its just this->direction = FLIP_X
...
void EggDriller::Car_Destroyed()
{
    ...
    this->position.y += this->velocity.y;
    if (!this->TileCollision(Zone::sVars->collisionLayers, CMODE_FLOOR, this->collisionPlane, 0, 4 << 16,
                             0)) { // need this check here, as if theres none the velocity keeps getting added anyway
        this->velocity.y +=
            0x2000; // this adds the velocity every frame so the car slowly goes more and more down until it hits the ground and its set to 0
    }
}
...
void EggDriller::Wheel_Bounce()
{
    ...
    this->position.x += this->velocity.x;
    this->position.y += this->velocity.y;
    this->velocity.y += 0x3800; // this adds the velocity every frame so the wheel slowly goes more and more down until it hits the ground and the
                                // velocity gets reversed
